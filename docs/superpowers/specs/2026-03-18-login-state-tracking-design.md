...
### Scope Of Analytics Enrichment

- ログイン文脈の付与対象は、既存のページ表示イベントと今回追加する `login_success` / `logout` に限定する
- 既存の CTA、リード送信、購入などの非ログイン系イベントは今回の対応では変更しない
- 将来的に全イベントへの付与が必要になった場合は、別タスクとして拡張する
...
- 既存の CTA、リード送信、購入イベントの送信仕様は今回の対応で変更されない
